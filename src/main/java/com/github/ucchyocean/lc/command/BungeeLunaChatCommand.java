...
public class BungeeLunaChatCommand extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if ( args.length >= 1 ) {
            for ( SubCommandAbst c : commonCommands ) {
                if ( c.getCommandName().equalsIgnoreCase(args[0]) ) {
                    ...
                    // 実行
                    c.runCommand(new CommandSenderBungee(sender), getName(), args);
                    return;
                }
            }
        }
        ...
        // 引数なしは、ヘルプを表示
        if (args.length == 0) {
            helpCommand.runCommand(new CommandSenderBungee(sender), getName(), args);
            return;
        }
        ...
        joinCommand.runCommand(new CommandSenderBungee(sender), getName(), args);
    }
    ...
    public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
        if ( args.length == 1 ) {
            ...
            for ( String name : getListCanJoin(new CommandSenderBungee(sender)) ) {
            ...
            String arg = args[1].toLowerCase();
            ArrayList<String> items = new ArrayList<String>();
            for ( String name : getListCanRemove(new CommandSenderBungee(sender)) ) {
                if ( name.toLowerCase().startsWith(arg) ) {
                    items.add(name);
                }
            }
